...
import { createCandidateCv, createCandidateCvWithImg } from "../../apis/cv";
import "./createCv.css"
...
const CreateCvScene = ({ isLoggedIn }) => {
    ...
    const [awardLength, setAwadLength] = useState(0);
    ...
    const fetchCandidateData = async () => {
        try {
            ...
            setCvImg(localStorage.getItem('avatar'))
    ...
    const handleCreateCv = async (e) => {
        ...
        try {
            if (isUseAvatar) {
                ...
                const cvImgUrl = localStorage.getItem('avatar');
                const result = await createCandidateCv({
                    cvTitle,
                    fullName,
                    email,
                    phoneNumber,
                ...
                const result = await createCandidateCvWithImg(cvData);
    ...
    const handleSubmitAward = async (awardData, index) => {
        if (index === -1) {
            ...
            setAwadLength((prevLength) => prevLength + 1);
    ...
    return (
        <div className="create-cv-body">
            <h1 className="create-cv-title">Tạo CV mới</h1>
            <form onSubmit={handleCreateCv} className="form-create-cv">
                ...
                <button disabled={isSubmitting} id="submitCvBtn" type="submit">{isSubmitting ? <i className="fa fa-spinner fa-spin"></i> : "Tạo CV"}</button>
            </form>
            <ToastContainer />
        </div>
    )
